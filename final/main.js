...
function exefiletrack() {
  woopra.track("downloaded .exe file", { amount: "-" });
}

function trackfcbclick() {
  woopra.track("clicked on facebook page", { amount: "-" });
}

function trackgithubclick() {
  woopra.track("clicked on github page", { amount: "-" });
}

function trackblogclick() {
  woopra.track("clicked on michaelcukier.com", { amount: "-" });
}

function itworks() {
  woopra.track("said it works", { amount: "-" });
...
function itdoesntwork() {
  woopra.track("said it doesnt work", { amount: "-" });
  $(".it_works").attr('visibility', 'hidden');
  $(".it_doesnt_work").attr('visibility', 'hidden');
  $(".isitworking").text("Thanks for your feedback!");
}
...
$('#upload-input-1').on('change', function(){
  ...
  if (files.length > 0){
    ...
    if (regexcheckname(file.name) == true) {
      ...
      pyfile_step1 = false;
    } else if ((file.name).indexOf(".py") > -1) {
      $.ajax({
        url: '/upload',
        type: 'POST',
        data: formData,
...
$('#upload-input-3').on('change', function(){
  ...
  if (files.length > 0){
    ...
    } else if ((file.name).indexOf(".ico") > -1) {
      $.ajax({
        url: '/upload',
        type: 'POST',
        data: formData,
...
$('.convbut').on('click', function (){
  ...
  if (blockconvert == true) {
    ...
    woopra.track('started one convertion...', { amount: uniqueRoom + "/" + $('.fileselector1').text() + " - addfiles: " + $('.fileselector2').text() + " - libs: " + $('#myTags').tagit('assignedTags')});
    ...
    socket.on('error_happenned', function(data){
       ...
       swal({
         title: "Error!",
         text: "Unexpected error: please make sure you're using Python 3 & you're importing the correct libraries.",
         type: "error",
         showCancelButton: false,
    ...
    socket.on('step4', function(data){
       ...
       $(".isitworking").append("<hr> Help us improve: <br><br> <button class='it_works' onClick='itworks()'> my .exe file works </button> <button class='it_doesnt_work' onClick='itdoesntwork()'> my .exe file doesnt work </button>")
